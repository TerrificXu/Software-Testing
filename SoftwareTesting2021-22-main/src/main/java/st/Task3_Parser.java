...
public class Task3_Parser {
	...
	public void addAll(String options, String shortcuts, String types) {
		...
		if (options == null || shortcuts == null || types == null) {
			return;
		}

		String[] optionNames = separateSpace(options);
		String[] shortcutNames = separateSpace(shortcuts);
		String[] typeNames = separateSpace(types);

		if (typeNames.length == optionNames.length) {
			if (shortcutNames.length == optionNames.length) {
				for (int i = 0; i < optionNames.length; i++) {
					addOption(new Option(optionNames[i], transType(typeNames[i].trim())), shortcutNames[i]);
				}
			}

		} else if (typeNames.length < optionNames.length&&typeNames.length==1) {
			for (String optionName : optionNames) {
				addOption(new Option(optionName, transType(typeNames[0].trim())));
			}
		}
	...
	public void addAll(String options, String types) {
		...
		if (typeNames.length == optionNames.length) {
			for (int i = 0; i < optionNames.length; i++) {
				addOption(new Option(optionNames[i], transType(typeNames[i].trim())));
			}
	...
	public void addAll(String group) {
		if (isGroup(group)) {
			char[] chars = group.toCharArray();
			String name = group.substring(0, group.length() - 3);
			char nlast=chars[chars.length-1];
			char nfirst=chars[chars.length-3];
			if(Character.isDigit(nfirst)&&Character.isDigit(nlast)){
				int range1=Character.getNumericValue(nfirst);
				int range2=Character.getNumericValue(nlast);
				if(range1>range2){
					for(int i=range2;i<=range1;i++){
						addOption(new Option(name+i,Type.STRING));
					}
				}
			}
	...
	public Type transType(String type) {
		switch (type) {
			case "Integer":
				return Type.INTEGER;
			case "String":
	...
	public String[] separateSpace(String values) {
